...
class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    update() {
        ...
        if (this.name === 'Player 1') {
            const { A, D, W } = this.scene.input.keyboard.addKeys('W,S,A,D');
            left = A;
            right = D;
            up = W;
        }
    ...
    handleAttacks() {
        var projectileKey = 'keydown-K';
        var meleeKey = 'keydown-L';
        if (this.name === 'Player 1') {
            projectileKey = 'keydown-G';
            meleeKey = 'keydown-H';
        }
        this.scene.input.keyboard.on(projectileKey, () => {
            this.projectileLaunchSound.play();
            this.play('throw', true)
            this.projectiles.fireProjectile(this, 'iceball');
        })

        this.scene.input.keyboard.on(meleeKey, () => {
    ...
    handleMovements() {
        ...
        this.scene.input.keyboard.on(`keydown-${slideKey}`, () => {
        ...
        this.scene.input.keyboard.on(`keyup-${slideKey}`, () => {
        ...
        if (source.body) {
                ...
                this.body.blocked.right ?
                    ...
                    this.body.blocked.down ?
                        this.body.x > this.scene.config.width / 2 ?
                            this.setVelocity(-this.bounceVelocity) :
